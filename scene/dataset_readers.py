...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image_path=image_path, image_name=image_name, width=width, height=height)
...
def fetchPly(path):
    ...
    normals = np.zeros_like(positions) #np.vstack([vertices['nx'], vertices['ny'], vertices['nz']]).T
...
def readMegaNeRFBuildingInfo(path, images, eval):
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
...
sceneLoadTypeCallbacks = {
    ...
    "MegaNeRFBuilding": readMegaNeRFBuildingInfo
}
